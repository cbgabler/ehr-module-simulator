...
Consider implementing automated backups in the application:
...
function createBackup() {
  ...
  const backupPath = path.join(backupDir, `ehr_scenarios_${timestamp}.db`);
  ...
  const backups = fs.readdirSync(backupDir)
    .filter(f => f.startsWith('ehr_scenarios_'))
    .sort()
    .reverse();
...
1. **Test locally first** - Run schema changes on a test database
2. **Create backups** - Before deployments or major changes
3. **Use transactions** - Wrap related changes in transactions
4. **Validate data** - Check constraints before inserting

```javascript
// Example: Using transactions for safety
const insertData = db.transaction(() => {
  db.prepare('INSERT INTO users (username, role) VALUES (?, ?)').run('john', 'student');
  db.prepare('INSERT INTO sessions (userId, scenarioId) VALUES (?, ?)').run(1, 1);
});
